/**
 * QA: Full Acme Walkthrough — all 10 stages
 ...
 * - Stage 6 outcomes with baseline/target
          ...
          where: vi.fn().mockReturnValue({
            limit: vi.fn().mockResolvedValue([{
              stageGateStateJson: JSON.stringify({
                stage1: { completedAt: Date.now() - 30 * 86400000 },
                stage2: { completedAt: Date.now() - 25 * 86400000 },
                stage3: { completedAt: Date.now() - 20 * 86400000 },
                stage4: { completedAt: Date.now() - 15 * 86400000 },
                stage5: { completedAt: null },
                stage6: { completedAt: null },
                stage7: { completedAt: null },
                stage8: { completedAt: null },
                stage9: { completedAt: null },
                stage10: { completedAt: null },
              }),
            }]),
          }),
        }),
...
// ─── Stage 6: Outcomes ────────────────────────────────────────────────────────
describe("QA: Stage 6 — outcomes with baseline/target", () => {
  it("completeStage6 accepts 3 outcomes with baseline and target values", async () => {
    const mockDb = {
      select: vi.fn().mockReturnValue({
        from: vi.fn().mockReturnValue({
          where: vi.fn().mockReturnValue({
            limit: vi.fn().mockResolvedValue([{
              stageGateStateJson: JSON.stringify({
                ...
                stage3: { completedAt: Date.now() - 20 * 86400000 },
                stage4: { completedAt: Date.now() - 15 * 86400000 },
                stage5: { completedAt: Date.now() - 12 * 86400000 },
                stage6: { completedAt: null },
                stage7: { completedAt: null },
                stage8: { completedAt: null },
                stage9: { completedAt: null },
    ...
    expect(result.ok).toBe(true);
    console.log("Stage 6 completed. Outcomes:");
...
describe("QA: Stage 7 — business case narrative", () => {
  it("completeStage7 accepts the business case narrative (>50 words)", async () => {
    const mockDb = {
      select: vi.fn().mockReturnValue({
        from: vi.fn().mockReturnValue({
          where: vi.fn().mockReturnValue({
            limit: vi.fn().mockResolvedValue([{
              stageGateStateJson: JSON.stringify({
                ...
                stage4: { completedAt: Date.now() - 15 * 86400000 },
                stage5: { completedAt: Date.now() - 12 * 86400000 },
                stage6: { completedAt: Date.now() - 10 * 86400000 },
                stage7: { completedAt: null },
                stage8: { completedAt: null },
                stage9: { completedAt: null },
                stage10: { completedAt: null },
    ...
    console.log(`Stage 7 completed. Business case narrative: ${wordCount} words`);
    console.log(`\n${ACME_BUSINESS_CASE}`);
  });
});
          ...
          where: vi.fn().mockReturnValue({
            limit: vi.fn().mockResolvedValue([{
              stageGateStateJson: JSON.stringify({
                ...
                stage5: { completedAt: Date.now() - 12 * 86400000 },
                stage6: { completedAt: Date.now() - 10 * 86400000 },
                stage7: { completedAt: Date.now() - 8 * 86400000 },
                stage8: { completedAt: null },
                stage9: { completedAt: null },
                stage10: { completedAt: null },
              }),
...
// ─── Stage 9: Tensions ────────────────────────────────────────────────────────
describe("QA: Stage 9 — tensions and gate completion", () => {
  ...
  it("completeStage9 sets reviewHeldAt and clears Stage 9 gate", async () => {
          ...
          where: vi.fn().mockReturnValue({
            limit: vi.fn().mockResolvedValue([{
              stageGateStateJson: JSON.stringify({
                ...
                stage6: { completedAt: Date.now() - 10 * 86400000 },
                stage7: { completedAt: Date.now() - 8 * 86400000 },
                stage8: { completedAt: Date.now() - 5 * 86400000 },
                stage9: { completedAt: null },
                stage10: { completedAt: null },
              }),
            }]),
    ...
    const caller = appRouter.createCaller(makeCtx());
    const result = await caller.gate.completeStage9({ reviewHeldAt });

    expect(result.ok).toBe(true);
    expect(result.gateState.stage9.completedAt).toBeTruthy();
    console.log(`\nStage 9 gate cleared. reviewHeldAt: ${new Date(reviewHeldAt).toISOString()}`);
    console.log(`stage9.completedAt: ${new Date(result.gateState.stage9.completedAt!).toISOString()}`);
  });
});

// ─── Stage 10: Board report ───────────────────────────────────────────────────
describe("QA: Stage 10 — board report and gate completion", () => {
  it("completeStage10 accepts 6 sections with total word count in 1200-4000 range", async () => {
    const mockDb = {
      select: vi.fn().mockReturnValue({
        from: vi.fn().mockReturnValue({
          where: vi.fn().mockReturnValue({
            limit: vi.fn().mockResolvedValue([{
              stageGateStateJson: JSON.stringify({
                ...
                stage7: { completedAt: Date.now() - 8 * 86400000 },
                stage8: { completedAt: Date.now() - 5 * 86400000 },
                stage9: { completedAt: Date.now() - 2 * 86400000 },
                stage10: { completedAt: null },
              }),
            }]),
          }),
  ...
  it("completeStage10 rejects if any section is missing", async () => {
    const mockDb = {
      select: vi.fn().mockReturnValue({
        from: vi.fn().mockReturnValue({
          where: vi.fn().mockReturnValue({
            limit: vi.fn().mockResolvedValue([{
              stageGateStateJson: JSON.stringify({ stage9: { completedAt: Date.now() - 2 * 86400000 }, stage10: { completedAt: null } }),
            }]),
          }),
        }),
      }),
    ...
    const caller = appRouter.createCaller(makeCtx());
    await expect(caller.gate.completeStage10({
...
describe("QA: Gate timestamps summary", () => {
  it("prints all gate completion timestamps for the Acme walkthrough", () => {
    ...
    expect(Object.keys(timestamps)).toHaveLength(10);
