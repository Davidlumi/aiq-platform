...
**Version:** 4.0  
**Date:** 29 May 2026 (amended v2.1 brief)  
**Remediation items:** Fix 14 (P1) — Addendum R2; R3 amendment — add `legacy_key` column, pin migration timestamp; v2 brief amendment — exact-timestamp SQL, prod-DB framing; v2.1 amendment — split legacy-key inference into `script_mapped` vs `title_inferred` columns  
...
**v2.1 amendment:** The `legacy_key` column is now split into two columns:
- `script_mapped`: The legacy key is uniquely determined by the REMAP constant (only one legacy key maps to this canonical key). **Deterministic — no ambiguity.**
- `title_inferred`: Two legacy keys map to the same canonical key (`appropriateness` and `governance` both map to `ai_ethics_trust`). The assignment is inferred from the row title. **Indicative only — not verifiable from DB.**

| Row ID | `script_mapped` | `title_inferred` | Current `capability_key` | Title |
|---|---|---|---|---|
...
**Note:** All 10 Group 1 rows are `title_inferred` (not `script_mapped`) because two legacy keys (`appropriateness` and `governance`) both map to `ai_ethics_trust` in the REMAP constant. The per-row assignment cannot be determined from the script alone.
...
**v2.1 amendment:** Same two-column split as Group 1. Both `data_interpretation` and `judgement` map to `ai_output_evaluation`, so all 11 rows are `title_inferred`.
...
**v2.1 amendment:** Same two-column split. Both `workflow` and `execution` map to `ai_workflow_design`, so all 5 rows are `title_inferred`.
...
The addendum's request to "add the `legacy_key` column" has been met by this inference.

**v2.1 amendment — `script_mapped` vs `title_inferred` distinction:**

The v2.1 brief requires the audit trail to distinguish between:

- **`script_mapped`** (deterministic): The legacy key is uniquely determined by the REMAP constant because only one legacy key maps to that canonical key. No ambiguity. The assignment is verifiable from the script alone.
- **`title_inferred`** (indicative): Two legacy keys map to the same canonical key. The per-row assignment is inferred from the row title and content. This inference is not verifiable from the DB or script alone.

In this migration, **all 26 rows are `title_inferred`** because every canonical key in the REMAP constant has two legacy keys mapping to it:
- `ai_ethics_trust` ← `appropriateness` + `governance`
- `ai_output_evaluation` ← `data_interpretation` + `judgement`
- `ai_workflow_design` ← `workflow` + `execution`

There are **zero `script_mapped` rows** in this migration. The `script_mapped` column is present in the tables above for completeness and to support future migrations where a canonical key has only one legacy key mapping to it.

A content owner reviewing the 26 rows should treat all legacy key assignments as indicative, not authoritative.
...
| 4.0 | 29 May 2026 | v2.1 amendment: split `legacy_key` column into `script_mapped` vs `title_inferred`; confirmed all 26 rows are `title_inferred` (zero `script_mapped`); updated Group 1/2/3 tables; expanded Limitation section |
