...
const PRIORITY_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  ...
  low: { label: "Low priority", bg: "#F0FDF4", text: "#047857" },
};
...
function PromptCard({ prompt }: {
  ...
  // Generate a suggested opening script based on the pattern
  ...
  return (
    <div className="bg-white rounded-xl border border-neutral-100 shadow-sm p-5">
      ...
      <div className="flex items-start gap-4 mb-4">
        <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium flex-shrink-0" style={{ background: "#E0E7EF", color: "#1F3A5F" }}>
          {initials}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <p className="text-sm font-semibold" style={{ color: "#0F2547" }}>{prompt.memberName}</p>
      ...
      {/* Observation */}
      <div className="mb-3">
        <p className="text-xs font-medium uppercase tracking-widest mb-1.5" style={{ color: "#6B7280" }}>What the data shows</p>
        <p className="text-sm leading-relaxed" style={{ color: "#0F2547" }}>{prompt.observation}</p>
      </div>

      {/* Suggested action */}
      <div className="mb-3 p-3 rounded-lg" style={{ background: "#F8FAFC", border: "0.5px solid #E5E7EB" }}>
        <p className="text-xs font-medium uppercase tracking-widest mb-1" style={{ color: "#6B7280" }}>Suggested action</p>
        <p className="text-sm" style={{ color: "#0F2547" }}>{prompt.suggestedAction}</p>
      </div>

      {/* Opening script */}
      <div className="p-3 rounded-lg" style={{ background: "#EFF6FF", border: "0.5px solid #BFDBFE" }}>
        <p className="text-xs font-medium uppercase tracking-widest mb-1.5" style={{ color: "#1D4ED8" }}>Suggested opening</p>
        <p className="text-sm italic leading-relaxed" style={{ color: "#1E40AF" }}>{script}</p>
      </div>
    </div>
  );
}
    ...
    <div className="px-5 py-6 md:px-8 max-w-3xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-neutral-100">
        <div>
          ...
          <h1 className="text-lg font-semibold" style={{ color: "#0F2547" }}>Conversation Prompts</h1>
          <p className="text-xs mt-1" style={{ color: "#6B7280" }}>Data-driven prompts to guide meaningful 1:1 conversations</p>
        </div>
        <Link href="/dashboard">
      ...
      {!isLoading && (!data?.prompts || data.prompts.length === 0) && (
        <div className="bg-white rounded-xl border border-neutral-100 shadow-sm p-10 flex flex-col items-center gap-3 text-center">
          <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ background: "#F3F4F6" }}>
            <MessageSquare className="w-6 h-6" style={{ color: "#9CA3AF" }} />
          </div>
          <p className="text-sm font-medium" style={{ color: "#0F2547" }}>No prompts available yet</p>
